...
void main() {
  var stars = <Star>[];
  for (var value in regex.allMatches(input2)) {
    stars.add(Star(num(value, 2), num(value, 1), num(value, 4), num(value, 3)));
  }

  for (var i = 0; i < 10036; i++) {
    stars.forEach((s) => s.step());
  }
  printGrid(stars); // part 1
  // turns out that was also the part 2 answer
}
...
int num(Match match, int group) {
  return int.parse(match.group(group));
}
